        ...
        switch (command) {
            ...
            case 'menu': {
                if (cekUser("id", sender) == null) {
                    return liwirya.sendMessage(from, {
                        text: `🚫 Maaf, Anda belum terdaftar sebagai pengguna. Silakan hubungi owner di @${global.ownerNumber} untuk mendaftar!`,
                ...
                const userName = db_user.find(user => user.id === sender)?.name || `User${totalUsers}`;
                let object_user = {
                    id: sender,
                    name: userName,
                    premium: false
                ...
                const totalTransaksi = riwayatTransaksi[sender]?.length || 0;
...
╰───────────────╯`;
                ...
                if (isGroup) {
                    const groupMenuText = `
${header}
🌈 *Fitur lengkap tersedia di private chat!*
${contactInfo}
🔥 Kirim pesan ke bot di *private chat* untuk menikmati semua fitur kami!`;
...
${contactInfo}`;
                    ...
                    const menuSections = [{
                            ...
                            rows: [{
                                    ...
                                    rowId: "#pricelist",
                                    description: "Cek daftar produk digital terbaru"
                                },
                                {
                                    title: "💳 Dompet Digital",
                                    rowId: "#dompet",
                                    description: "Kelola deposit, transfer, dan riwayat transaksi"
                                },
                                {
                                    title: "🏆 Top Saldo",
                                    rowId: "#topsaldo",
                                    description: "Lihat pengguna dengan saldo tertinggi"
                                }
                            ]
                        },
                        {
                            ...
                            rows: [{
                                ...
                                },
                                {
                                    title: "💡 Saran",
                                    rowId: "#saran",
                                    description: "Berikan ide untuk tingkatkan bot"
                                }
                            ]
                        },
                        {
                            title: "ℹ️ Informasi Lain",
                            rows: [{
                                    title: "📜 Syarat & Ketentuan",
                                    rowId: "#syarat",
                                    description: "Baca aturan penggunaan bot"
                                },
                                {
                                    title: "❓ Bantuan",
                                    rowId: "#bantuan",
                                    description: "Hubungi tim support jika ada masalah"
                                }
                            ]
                        },
                        {
                    ...
                    const messageOptions = {
                        ...
                        image: {
                            url: 'https://qu.ax/sAQiY.jpg'
                        }, //Tdk support Image
                        caption: menuText
                    };

                    liwirya.sendMessage(from, messageOptions, {
                        quoted: msg
                    });
                }
                break;
            ...
            case 'saldo': {
                if (cekUser("id", sender) == null) {
                    return await liwirya.sendMessage(from, {
                        text: `🚫 *Maaf, kamu belum terdaftar!* Silakan daftar terlebih dahulu atau hubungi owner di @${global.ownerNumber.split('@')[0]}.`,
                        mentions: [global.ownerNumber, sender]
                    }, {
                        quoted: fkontak
                    });
        ...
        `;
            ...
            case 'topsaldo': {
                try {
                    ...
                    const topSaldo = db_saldo.sort((a, b) => b.saldo - a.saldo).slice(0, 10);
                    if (topSaldo.length === 0) return reply('📉 *Daftar top saldo kosong.*');

                    let teks = `
🏆 *Top 10 Saldo Pengguna* 🏆
📅 *Tanggal*: ${hari}, ${tglFormat}
🕛 *Waktu*: ${jam}
...
📋 *Peringkat Saldo*:
        `;
                    topSaldo.forEach((user, index) => {
                        teks += ` ${index + 1}. User Top ${index + 1} - Rp${toRupiah(user.saldo)}\n`;
                    });
                    teks += `
📌 *Catatan*:
- Data saldo pengguna disimpan secara aman.
- Nomor pengguna tidak ditampilkan demi keamanan.
        `;
                    ...
                    await reply('❌ *Error:* Terjadi kesalahan saat memuat top saldo. Silakan coba lagi.');
                }
                break;
            }
            break;
            ...
            case 'addsaldo': {
                if (!isOwner) {
                    return reply('🚫 *Akses Ditolak!* Hanya owner yang dapat menggunakan perintah ini.');
                }

                if (!q || !q.includes('|')) {
                    return reply(`📝 *Cara Penggunaan:*\n${prefix + command} Nomor|Jumlah\n\n*Contoh:*\n${prefix + command} 6283141471040|20000\n⚠️ Pastikan nomor dan jumlah diisi dengan benar!`);
                }

                const [phoneNumber, amount] = q.split('|');

                if (!phoneNumber || !/^\d+$/.test(phoneNumber)) {
                    return reply('❌ *Error:* Nomor telepon tidak valid! Gunakan format angka, contoh: 6283141471040');
                }
                if (!amount || !/^\d+$/.test(amount)) {
                    return reply('❌ *Error:* Jumlah saldo tidak valid! Gunakan format angka, contoh: 20000');
                }

                const userId = `${phoneNumber}@s.whatsapp.net`;
                const saldoAmount = Number(amount);

                try {
                    addSaldo(userId, saldoAmount, db_saldo);
                    await sleep(50);

                    await liwirya.sendTextMentions(from, `✅ *Penambahan Saldo Berhasil!*\n\n` +
                        `⭔ *ID:* ${phoneNumber}\n` +
                        `⭔ *Nomor:* @${phoneNumber}\n` +
                        `⭔ *Tanggal:* ${tanggal}\n` +
                        `⭔ *Saldo Sekarang:* Rp${toRupiah(cekSaldo(userId, db_saldo))}`, [userId]);

                    await liwirya.sendTextMentions(userId, `🎉 *Saldo Anda Bertambah!*\n\n` +
                        `⭔ *ID:* ${phoneNumber}\n` +
                        `⭔ *Nomor:* @${phoneNumber}\n` +
                        `⭔ *Tanggal:* ${tanggal}\n` +
                        `⭔ *Saldo Sekarang:* Rp${toRupiah(cekSaldo(userId, db_saldo))}`, [userId]);
                    ...
                    await reply('⚠️ *Gagal!* Terjadi kesalahan saat menambahkan saldo. 😔');
                }
                break;
            }
            ...
            case 'minsaldo': {
                ...
                if (cekSaldo(userId, db_saldo) === 0) {
                    return reply('❌ *Error:* Pengguna belum terdaftar di database saldo!');
                }

                if (cekSaldo(userId, db_saldo) < saldoAmount) {
                    return reply(`⚠️ *Error:* Saldo pengguna hanya Rp${toRupiah(cekSaldo(userId, db_saldo))}. Jangan melebihi jumlah tersebut!`);
                }

                try {
                    minSaldo(userId, saldoAmount, db_saldo);
                    await sleep(50);

                    await liwirya.sendTextMentions(from, `✅ *Pengurangan Saldo Berhasil!*\n\n` +
                        `⭔ *ID:* ${phoneNumber}\n` +
                        `⭔ *Nomor:* @${phoneNumber}\n` +
                        `⭔ *Tanggal:* ${tanggal}\n` +
                        `⭔ *Saldo Sekarang:* Rp${toRupiah(cekSaldo(userId, db_saldo))}`, [userId]);
                    ...
                    await reply('⚠️ *Gagal!* Terjadi kesalahan saat mengurangi saldo. 😔');
                }
                break;
            }
            break
            ...
            case 'setprofit': {
                ...
                if (!q) {
                    return reply(`📝 *Cara Penggunaan:*\n${prefix + command} <persentase>%\n\n*Contoh:* ${prefix + command} 20%\n⚠️ Ganti *20%* dengan persentase keuntungan yang diinginkan (1%-99%).`);
                }

                const profitPercentage = parseInt(q.replace(/[^0-9]/g, ''));

                if (profitPercentage < 1) {
                    return reply('❌ *Error:* Persentase minimal adalah *1%*!');
                }

                if (profitPercentage > 99) {
                    return reply('❌ *Error:* Persentase maksimal adalah *99%*!');
                }

                try {
                    fs.writeFileSync('./database/profit', profitPercentage.toString());
                    await reply(`✅ *Sukses!* Profit Anda telah diatur menjadi *${profitPercentage}%*. 🎉`);
                } catch (error) {
                    console.error('Error writing to profit database:', error);
                    await reply('⚠️ *Gagal!* Terjadi kesalahan saat menyimpan pengaturan profit. 😔');
                }
                break;
            }
            break
            // menu pricelist and produk
            case 'pricelist': {
                if (isGroup) return await reply('🚫 *Maaf, perintah ini hanya bisa digunakan di private chat (PM)!*');
                if (cekUser("id", sender) == null) {
                    return await liwirya.sendMessage(from, {
                ...
                const requiredGroups = [{
                        id: '120363357801694436@g.us',
                        link: 'https://chat.whatsapp.com/G2wtnSYYBv5IyYgrmpsuWx'
                    },
                    {
                        id: '120363357801694436@g.us',
                        link: 'https://chat.whatsapp.com/LWROk9bvMiJITsySlCptQB'
                    }
                ];
                ...
                for (const group of requiredGroups) {
                    try {
                        const groupMetadata = await liwirya.groupMetadata(group.id);
                        const participants = groupMetadata.participants.map(p => p.id);
                        if (!participants.includes(sender)) {
                            isMember = false;
                            missingGroups.push(group.link);
                        }
                    } catch (error) {
                        console.error(`Error checking group ${group.id}:`, error);
                        return await liwirya.sendMessage(from, {
                            text: `❌ *Terjadi kesalahan saat memeriksa keanggotaan grup.* Silakan coba lagi nanti atau hubungi owner di @${global.ownerNumber.split('@')[0]}.`,
                            mentions: [global.ownerNumber]
                        }, {
                            quoted: fkontak
                        });
...
Untuk menggunakan fitur *${prefix}pricelist*, kamu harus bergabung ke grup berikut:

${missingGroups.map((link, i) => ` ${i + 1}. ${link}`).join('\n')}

📌 Gabung dulu ya, lalu coba lagi! Jika perlu bantuan, hubungi owner di @${global.ownerNumber.split('@')[0]}.
        `;
                    return await liwirya.sendMessage(from, {
                        text: replyText,
                        mentions: [global.ownerNumber]
                ...
                const listMessage = {
                    text: `
🌟 *${global.namaStore.toUpperCase()} - Daftar Produk* 🌟\n
📅 *Tanggal*: ${hari}, ${tglFormat} 
🕛 *Jam*: ${jam} 
👤 *Nama*: ${pushname}
💸 *Saldo*: Rp${toRupiah(cekSaldo(sender, db_saldo))}
...
╰─────────────────╯
        `,
                    footer: `⚡ Total Produk: ${sections.reduce((acc, section) => acc + section.rows.length, 0)} | Status: *Ready Stock*
📞 Hubungi @${global.ownerNumber.split('@')[0]} untuk bantuan atau request produk lain!`,
            ...
            case 'daftar': {
                if (cekUser("id", sender) !== null) {
                    return reply('🚫 Kamu sudah terdaftar di database! Silakan ketik *Menu* untuk melanjutkan.');
                }

                const res_us = makeid(10);
                const diacuk = db_user.length + 1;
                const user_name = `LW${diacuk}`;
                const tanggal = new Date();
                const hari = tanggal.toLocaleDateString('id-ID', {
                    weekday: 'long'
                });
                ...
                const object_user = {
                    ...
                    seri: res_us,
                    premium: false,
                    registered_at: `${hari}, ${tglFormat} ${jam}`
                };
                ...
                const verify_teks = `
🌟 *SELAMAT DATANG DI ${global.namaStore}!* 🌟
...
🆔 *ID User*: ${res_us}
👤 *Nama User*: ${user_name}
🕒 *Waktu Pendaftaran*: ${hari}, ${tglFormat} ${jam}
──────────────────────

🎉 Anda telah resmi terdaftar di database *${global.namaStore}*!

💡 *Langkah Selanjutnya*:
Silakan ketik *Menu* untuk melihat semua fitur yang tersedia.
...
Terima kasih telah bergabung! 😊
    `;
            ...
            // Case untuk menampilkan daftar game yang tersedia untuk topup
            case 'topupgame': {
                if (cekUser("id", sender) == null) {
                    return await liwirya.sendMessage(from, {
                        text: `🚫 *Maaf, kamu belum terdaftar!* Silakan daftar terlebih dahulu dengan *${prefix}daftar* atau hubungi owner di @${global.ownerNumber.split('@')[0]}.`,
...
💡 *Cara Topup*: Pilih game di bawah, lalu ikuti instruksi.
            ...
            case 'dompet': {
                ...
                try {
                    const sections = [{
                        title: '💳 Kelola Dompet Digital',
                        rows: [{
                                title: '⚡️ Isi Ulang Saldo',
                                rowId: `${prefix}deposit`,
                                description: 'Lakukan deposit untuk menambah saldo.'
                            },
                            {
                                title: '💸 Penarikan Saldo',
                                rowId: `${prefix}narikmas`,
                                description: 'Tarik saldo ke metode pembayaran pilihan.'
                            },
                            {
                                title: '📊 Riwayat Transaksi',
                                rowId: `${prefix}riwayat`,
                                description: 'Lihat riwayat transaksi deposit dan penarikan.'
                            }
                        ]
                    }];
...
💡 *Pilih opsi di bawah untuk mengelola dompet Anda:*
            `,
                    ...
                    console.error('Error in dompet:', error);
                    await liwirya.sendMessage(from, {
                        text: `❌ *Terjadi kesalahan saat memuat menu dompet digital.* Silakan coba lagi atau hubungi owner di @${global.ownerNumber.split('@')[0]}.`,
                        mentions: [global.ownerNumber]
                    }, {
                        quoted: fkontak
                    });
